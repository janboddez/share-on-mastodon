...
class Post_Handler {
	...
	public function render_meta_box( $post ) {
		...
		if ( empty( $current_screen ) || ! in_array( $current_screen->post_type, (array) $this->options['post_types'] ) ) {
		...
		<div class="misc-pub-section">
			...
			<label><input type="checkbox" name="share_on_mastodon" value="1" <?php checked( in_array( get_post_meta( $post->ID, '_share_on_mastodon', true ), array( '', '1' ) ) ); ?>>
			<?php _e( 'Share on <b>Mastodon</b>', 'share-on-mastodon' ); ?></label>
		</div>
		<?php
	}
	...
	public function update_meta( $post_id, $post ) {
		...
		if ( ! isset( $_POST['_share_on_mastodon_meta_nonce'] ) || ! wp_verify_nonce( $_POST['_share_on_mastodon_meta_nonce'], basename( __FILE__ ) ) ) {
			// Nonce missing or invalid.
			return;
		}

		if ( ! in_array( $post->post_type, (array) $this->options['post_types'] ) ) {
			// Unsupported post type.
			return;
		}
		...
		if ( isset( $_POST['share_on_mastodon'] ) ) {
			...
			update_post_meta( $post_id, '_share_on_mastodon', '0' );
		}
	}
	...
	public function toot( $old_status, $new_status, $post ) {
		...
		if ( empty( $this->options['mastodon_host'] ) || ! wp_http_validate_url( $this->options['mastodon_host'] ) ||
			 empty( $this->options['mastodon_access_token'] ) ) {
		...
		$status = strip_tags( get_the_title( $post->ID ) ) . ' ' . esc_url_raw( get_permalink( $post->ID ) );
		...
		if ( has_post_thumbnail( $post->ID ) ) {
			...
			$media_id = $this->_upload_thumbnail( $post->ID );

			if ( ! empty( $media_id ) ) {
				// Handle after `http_build_query()`, as apparently Mastodon
				// doesn't like numbers for query string array keys.
				$query_string .= '&media_ids[]=' . urlencode( $media_id );
			}
		}

		$ch = curl_init();

		curl_setopt( $ch, CURLOPT_URL, esc_url_raw( $this->options['mastodon_host'] . '/api/v1/statuses' ) );
	 ...
	 */
	private function _upload_thumbnail( $post_id ) {
